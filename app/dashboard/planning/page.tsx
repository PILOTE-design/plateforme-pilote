...
const TYPE_CONFIG: Record<DayType, {
  ...
  travail: { label: 'Travail',       bg: '',              border: '',                             text: '',              dot: '',           defaultHours: 0, pdfColor: '' },
  conges:  { label: 'Congé payé',    bg: 'bg-sky-100',    border: 'border border-sky-200',        text: 'text-sky-800',  dot: 'bg-sky-400', defaultHours: 7, pdfColor: '#bae6fd' },
  maladie: { label: 'Arrêt maladie', bg: 'bg-red-100',    border: 'border border-red-200',        text: 'text-red-800',  dot: 'bg-red-400', defaultHours: 0, pdfColor: '#fecaca' },
  repos:   { label: 'Repos',         bg: 'bg-gray-50',    border: 'border border-dashed border-gray-200', text: 'text-gray-400', dot: 'bg-gray-300', defaultHours: 0, pdfColor: '#f3f4f6' },
}

const EMP_PALETTES = [
  { bg: 'bg-violet-100', border: 'border border-violet-200', lborder: 'border-l-4 border-l-violet-400', text: 'text-violet-900', dot: 'bg-violet-500', hex: '#8b5cf6', lightHex: '#ede9fe' },
  { bg: 'bg-pink-100',   border: 'border border-pink-200',   lborder: 'border-l-4 border-l-pink-400',   text: 'text-pink-900',   dot: 'bg-pink-500',   hex: '#ec4899', lightHex: '#fce7f3' },
  { bg: 'bg-sky-100',    border: 'border border-sky-300',    lborder: 'border-l-4 border-l-sky-400',    text: 'text-sky-900',    dot: 'bg-sky-500',    hex: '#0ea5e9', lightHex: '#e0f2fe' },
  { bg: 'bg-orange-100', border: 'border border-orange-200', lborder: 'border-l-4 border-l-orange-400', text: 'text-orange-900', dot: 'bg-orange-500', hex: '#f97316', lightHex: '#ffedd5' },
  { bg: 'bg-teal-100',   border: 'border border-teal-200',   lborder: 'border-l-4 border-l-teal-500',   text: 'text-teal-900',   dot: 'bg-teal-500',   hex: '#14b8a6', lightHex: '#ccfbf1' },
  { bg: 'bg-rose-100',   border: 'border border-rose-200',   lborder: 'border-l-4 border-l-rose-400',   text: 'text-rose-900',   dot: 'bg-rose-500',   hex: '#f43f5e', lightHex: '#ffe4e6' },
  { bg: 'bg-amber-100',  border: 'border border-amber-200',  lborder: 'border-l-4 border-l-amber-400',  text: 'text-amber-900',  dot: 'bg-amber-500',  hex: '#f59e0b', lightHex: '#fef3c7' },
  { bg: 'bg-indigo-100', border: 'border border-indigo-200', lborder: 'border-l-4 border-l-indigo-400', text: 'text-indigo-900', dot: 'bg-indigo-500', hex: '#6366f1', lightHex: '#e0e7ff' },
]
...
export default function PlanningPage() {
  ...
  // Keep ref in sync for blur handlers
  ...
  useEffect(() => {
    ...
    const close = (e: MouseEvent) => {
      ...
      if (!target.closest('[data-cell]')) setSelectedCell(null)
    }
  ...
  function getEntry(empId: string): PlanningEntry {
    return entriesRef.current[empId] ?? emptyEntry(empId, week, year)
  }

  function getEntryState(empId: string): PlanningEntry {
  ...
  function changeType(empId: string, jour: JourDB, newType: DayType) {
    ...
    const updated: PlanningEntry = {
      ...getEntry(empId),
      [typeKey]: newType,
      [jour]: newType === 'travail' ? currentH : cfg.defaultHours,
    }
  ...
  function exportPDF() {
    ...
    const dayHeaders = dates.map((d, i) => {
      const wk = i >= 5
      return `<th style="background:${wk ? '#94a3b8' : '#1E3A5F'};color:white;padding:7px 5px;font-size:10px;text-align:center;">${fmtD(d)}</th>`
    }).join('')
    ...
    const empRows = employees.map((emp, i) => {
      ...
      const cost = calcCost(entry, Number(emp.hourly_rate))
      ...
      return `<tr>
        <td style="padding:7px 10px;border-bottom:1px solid #e2e8f0;border-left:3px solid ${pal.hex};background:#fafafa;">
          <div style="display:flex;align-items:center;gap:7px;">
            <div style="width:26px;height:26px;border-radius:50%;background:${pal.hex};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
              <span style="color:white;font-size:9px;font-weight:700;">${initials(emp.name)}</span>
            </div>
        ...
        ${cells}
        <td style="padding:6px;text-align:center;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-weight:700;font-size:12px;color:${hasOT ? '#ea580c' : '#1e293b'};">${totalH.toFixed(1)}h</td>
        <td style="padding:6px;text-align:center;background:#f0fdf4;border-bottom:1px solid #e2e8f0;font-weight:700;font-size:12px;color:#15803d;">${cost.toFixed(2)} €</td>
      </tr>`
    }).join('')

    const legend = Object.entries(TYPE_CONFIG).map(([key, cfg]) => {
      const color = key === 'travail' ? '#8b5cf6' : cfg.pdfColor.replace('#', '')
      const bg = key === 'travail' ? '#ede9fe' : cfg.pdfColor
      return `<span style="display:inline-flex;align-items:center;gap:4px;margin-right:12px;"><span style="width:10px;height:10px;border-radius:2px;background:${bg};display:inline-block;"></span><span style="font-size:9px;color:#64748b;">${cfg.label || 'Travail'}</span></span>`
    }).join('')

    const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>Planning S${week}</title>
<style>@page{size:A4 landscape;margin:1.2cm 1.5cm}*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;color:#1e293b}table{width:100%;border-collapse:collapse}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style>
</head><body>
<div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:14px;padding-bottom:10px;border-bottom:2px solid #1E3A5F;">
  ...
  <div style="text-align:right;"><div style="font-size:10px;color:#64748b;">Coût main d'œuvre total</div><div style="font-size:16px;font-weight:800;color:#15803d;">${grandCost.toFixed(2)} €</div></div>
</div>
<table><thead><tr>
  <th style="background:#1E3A5F;color:white;padding:7px 10px;font-size:10px;text-align:left;width:150px;">Employé</th>
  ${dayHeaders}
  <th style="background:#1E3A5F;color:white;padding:7px 5px;font-size:10px;text-align:center;">Total</th>
  <th style="background:#1E3A5F;color:white;padding:7px 5px;font-size:10px;text-align:center;">Coût</th>
</tr></thead><tbody>${empRows}</tbody></table>
<div style="margin-top:12px;display:flex;align-items:center;gap:4px;">${legend}</div>
<p style="margin-top:8px;font-size:9px;color:#94a3b8;border-top:1px solid #e2e8f0;padding-top:6px;">Droit français : taux normal ≤ 35h · +25 % de 36-43h · +50 % au-delà · CP comptés 7h/jour · Généré via PILOTE</p>
</body></html>`
  ...
  return (
    <div className="min-h-screen bg-gray-50" onClick={() => setSelectedCell(null)}>
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-2 flex items-center gap-5">
        ...
        {([
          { key: 'travail', label: 'Travail', dotClass: 'bg-violet-400' },
          { key: 'conges',  label: 'Congé payé',    dotClass: 'bg-sky-400'    },
          { key: 'maladie', label: 'Arrêt maladie', dotClass: 'bg-red-400'    },
          { key: 'repos',   label: 'Repos',         dotClass: 'bg-gray-300'   },
        ] as const).map(t => (
          <div key={t.key} className="flex items-center gap-1.5">
            <div className={`w-2.5 h-2.5 rounded-sm ${t.dotClass}`} />
        ...
        <table className="w-full min-w-[860px]">
          <thead>
            <tr className="bg-white border-b border-gray-200">
              <th className="w-44 px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider sticky left-0 bg-white z-10">Employé</th>
              {weekDates.map((date, i) => {
                const isToday = date.getUTCDate() === today.getDate() && date.getUTCMonth() === today.getMonth() && date.getUTCFullYear() === today.getFullYear()
                const isWE = i >= 5
                return (
                  <th key={i} className={`px-1.5 py-3 text-center min-w-[110px] ${isToday ? 'bg-[#1E3A5F]' : isWE ? 'bg-gray-50' : ''}`}>
                    <div className={`text-xs font-bold uppercase tracking-wide ${isToday ? 'text-white' : isWE ? 'text-gray-400' : 'text-gray-500'}`}>{JOURS_SHORT[i]}</div>
                    <div className={`text-lg font-bold ${isToday ? 'text-white' : isWE ? 'text-gray-300' : 'text-gray-800'}`}>{date.getUTCDate()}</div>
                    <div className={`text-[10px] ${isToday ? 'text-blue-200' : 'text-gray-400'}`}>{date.toLocaleDateString('fr-FR', { month: 'short', timeZone: 'UTC' })}</div>
                  </th>
                )
              })}
              <th className="px-3 py-3 text-center text-xs font-semibold text-gray-400 uppercase tracking-wider w-20">Total</th>
              <th className="px-3 py-3 text-center text-xs font-semibold text-gray-400 uppercase tracking-wider w-24">Coût</th>
              <th className="w-10" />
            </tr>
          </thead>
          <tbody>
              ...
              employees.map((emp, empIdx) => {
                ...
                return (
                  <tr key={emp.id} className="border-b border-gray-100">
                    {/* Employee cell */}
                    <td className={`px-3 py-2 sticky left-0 bg-white z-10 ${pal.lborder}`}>
                      <div className="flex items-center gap-2">
                    ...
                    {JOURS_DB.map((jour, idx) => {
                      ...
                      const cellBdr   = type === 'travail' ? pal.border : TYPE_CONFIG[type].border
                      ...
                      return (
                        <td key={jour} className="px-1.5 py-1.5">
                          <div className="relative" data-cell="true" onClick={e => e.stopPropagation()}>
                            {/* Block */}
                            <div
                              className={`rounded-xl cursor-pointer transition-all hover:shadow-md ${cellBg} ${cellBdr} min-h-[68px] p-2 flex flex-col justify-between select-none`}
                              ...
                              {type === 'travail' ? (
                                <div onClick={e => e.stopPropagation()}>
                                  <input
                                    ...
                                    className={`w-full text-center font-bold text-xl bg-transparent focus:outline-none ${pal.text} placeholder-gray-300`}
                                    placeholder="—"
                                  />
                                  {hours > 0 && <div className={`text-[10px] text-center opacity-50 ${cellTxt}`}>heures</div>}
                                </div>
                              ) : (
                                <div className={`text-xs font-semibold text-center ${cellTxt} opacity-80 mt-1`}>
                                  {type === 'conges' ? '7h équiv.' : type === 'maladie' ? 'CPAM' : '—'}
                                </div>
                              )}
                            </div>
                            ...
                            {isSelected && (
                              <div
                                className="absolute top-full left-0 z-40 mt-1.5 bg-white rounded-2xl shadow-2xl border border-gray-100 p-1.5 min-w-[168px]"
            ...
            {employees.length > 0 && (
              <tr className="bg-gray-900">
                ...
                {JOURS_DB.map(jour => {
                  ...
                  return (
                    <td key={jour} className="px-1.5 py-3 text-center">
                      {dayH > 0
                ...
                <td className="px-3 py-3 text-center"><span className="font-bold text-white">{grandH.toFixed(1)}h</span></td>
      ...
      {employees.length > 0 && (
        <div className="px-6 py-2.5 bg-amber-50 border-t border-amber-100">
          <p className="text-xs text-amber-700">
            <span className="font-semibold">Droit français :</span> taux normal ≤ 35h · +25 % de 36-43h · +50 % au-delà · Congés payés comptés 7h/jour
          </p>
        </div>
      )}
