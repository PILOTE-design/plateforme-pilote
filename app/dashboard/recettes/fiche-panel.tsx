...
// Le CONTENU de la fiche recette façon Otami, en composant réutilisable :
...
// Bandeau de chiffres-clés + onglets Infos · Ingrédients · Fabrication · Vente.
// Les étapes de fabrication et l'argumentaire s'éditent SUR PLACE (PUT) ;
// « Modifier la fiche » délègue au parent (modale complète ou navigation).

import { useMemo, useState } from 'react'
import Link from 'next/link'
import { Pencil, Plus, X, Clock, ShoppingBasket, Package, AlertTriangle } from 'lucide-react'
...
export default function FichePanel({
  ...
  onEditFull: () => void
  /** Appelé après un enregistrement réussi (étapes/argumentaire) */
  onSaved: () => void
  ...
  const [steps, setSteps] = useState<string[]>(Array.isArray(recipe.fabrication_steps) ? recipe.fabrication_steps.map(String) : [])
  const [points, setPoints] = useState<string[]>(() => {
    const pts = Array.isArray(recipe.selling_points) ? recipe.selling_points.map(String) : []
  ...
  return (
    <div className="bg-white rounded-2xl border border-pilote-100 shadow-card-hover overflow-hidden">
      ...
      <div className="p-5">
        ...
        {c && (
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
            ...
            <div className="rounded-2xl bg-white border border-gray-100 shadow-card p-4">
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Prix de vente TTC</p>
              ...
              <p className="text-[11px] text-gray-400 mt-0.5">{c.pv_unitaire_ht !== null ? `${fmtEuro(c.pv_unitaire_ht)} HT` : 'à renseigner'}</p>
            </div>
            <div className="rounded-2xl bg-white border border-gray-100 shadow-card p-4">
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Marge brute</p>
              <p className="text-xl font-extrabold tracking-tight text-gray-900 tabular mt-1">
                {c.pv_unitaire_ht !== null && c.par_unite_ht !== null ? fmtEuro(c.pv_unitaire_ht - c.par_unite_ht) : '—'}
              </p>
              <p className="text-[11px] text-gray-400 mt-0.5">PV HT − coût</p>
            </div>
            <div className="rounded-2xl bg-white border border-gray-100 shadow-card p-4">
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Coef de marge</p>
            ...
            <div className="rounded-2xl bg-white border border-gray-100 shadow-card p-4">
              ...
              <p className="text-[11px] text-gray-400 mt-0.5">du PV HT</p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-1.5 mb-4 flex-wrap">
          {ONGLETS.map(o => (
            <button key={o} onClick={() => setTab(o)}
              className={`text-xs font-semibold rounded-full px-3.5 py-1.5 transition-colors ${tab === o ? 'bg-pilote text-white shadow-card' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}>
              {o}
            </button>
          ))}
        </div>
              ...
              <table className="w-full min-w-[640px]">
                  ...
                  {recipe.ingredients.map((ing, i) => {
                    ...
                    return (
                      <tr key={i} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-2.5 text-right text-sm text-gray-600 tabular">{poidsPct !== null ? `${Math.round(poidsPct)} %` : '—'}</td>
                        <td className="px-4 py-2.5 text-right text-sm font-semibold text-gray-900 tabular">{ing.unit_price_ht !== null ? fmtEuro(ing.line_total_ht) : '—'}</td>
                        <td className="px-4 py-2.5 text-right text-sm text-gray-600 tabular">{coutPct !== null && ing.unit_price_ht !== null ? `${Math.round(coutPct)} %` : '—'}</td>
                      </tr>
                    )
                  })}
                </tbody>
                <tfoot>
                  <tr className="bg-pilote text-white">
                    <td className="px-4 py-2.5 text-[11px] font-bold uppercase tracking-wider text-white/60">Total matière{c && c.emballage_ht > 0 ? ' + emballage' : ''}</td>
                    <td className="px-4 py-2.5 text-right font-bold tabular">{poidsTotalKg > 0 ? `${fmtQty(poidsTotalKg)} kg` : ''}</td>
                    <td className="px-4 py-2.5" />
                    <td className="px-4 py-2.5 text-right font-bold tabular">{fmtEuro(coutTotal)}</td>
                    <td className="px-4 py-2.5 text-right font-bold tabular text-white/70">100 %</td>
                  </tr>
                </tfoot>
              </table>
            </div>
            <div className="px-4 py-2.5 border-t border-gray-100 flex flex-wrap gap-x-6 gap-y-1 text-[11px] text-gray-500 tabular">
        ...
        {tab === 'Fabrication' && (
          <div>
            <p className="text-[11px] text-gray-400 mb-3">Étapes ordonnées — modifiables ici, puis « Enregistrer » en haut de l&apos;encadré</p>
            {steps.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-5">Veuillez renseigner le procédé de fabrication pour cette recette.</p>
            )}
            ...
            <button onClick={() => { setSteps(prev => [...prev, '']); setDirty(true) }}
              className="mt-3 w-full flex items-center justify-center gap-1.5 text-xs font-semibold text-gray-500 border-2 border-dashed border-gray-200 rounded-xl py-2.5 hover:border-pilote-200 hover:text-pilote transition-colors">
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-gray-100 p-4">
              <div className="flex items-center justify-between gap-2 mb-3">
                <h3 className="text-xs font-bold uppercase tracking-wider text-gray-400">Coût et vente</h3>
                <button onClick={onEditFull} className="p-1.5 rounded-lg text-gray-400 hover:text-pilote hover:bg-pilote-50 transition-colors" title="Modifier prix de vente et TVA">
              ...
              <div className="mt-3.5 flex items-center gap-2.5 flex-wrap">
                <span className="inline-flex items-center gap-2 bg-pilote-50 ring-1 ring-pilote-100 rounded-full px-3.5 py-1.5 text-sm font-bold text-pilote tabular">
                  PV TTC {recipe.selling_price_ttc != null ? fmtEuro(recipe.selling_price_ttc) : '—'}
                </span>
                <span className="inline-flex items-center gap-2 bg-green-50 ring-1 ring-green-100 rounded-full px-3.5 py-1.5 text-sm font-bold text-green-700 tabular">
                  coef {c?.coefficient !== null && c?.coefficient !== undefined ? `×${c.coefficient.toLocaleString('fr-FR')}` : '—'}
                </span>
                <span className="inline-flex items-center gap-2 bg-pilote text-white rounded-full px-3.5 py-1.5 text-sm font-bold tabular">
            ...
            <div className="rounded-2xl border border-gray-100 p-4">
              <h3 className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Argumentaire de vente</h3>
              <p className="text-[11px] text-gray-400 mb-3">Les arguments à donner en boutique — modifiables ici, puis « Enregistrer » en haut de l&apos;encadré</p>
